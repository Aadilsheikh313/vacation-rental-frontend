...
import styles from "../../adminStylesModule/Guest/admindailyactive.module.css";
...
import { getAllDailyActiveGuest } from "../../config/redux/action/adminGuestAction";
import CustomSpinner from "../../comman/Spinner";

const AdminGetDailyActiveGuest = () => {
    ...
    useEffect(() => {
        dispatch(getAllDailyActiveGuest());
    ...
    return (
        <div className={styles.GetDailyActiveGuestcontainer}>
            <div className={styles.tableWrapper}>
                <Card className={styles.summaryCard}>
                    <Card.Body>
                        <h3>Total Daily Active Registered Guest : <span>{dailyActiveGuestsCount}</span></h3>
                ...
                {isSuccess && dailyActiveGuests.length === 0 && (
                    <p className={styles.noGuestsText}>No Today Actvie guests found.</p>
                )}

                {!isLoading && dailyActiveGuests.length > 0 && (
                    <div className={styles.tableWrapper}>
                        <table className={styles.table}>
                            ...
                            <tbody>
                                {dailyActiveGuests.map((guest, index) => (
                                    <tr key={guest._id}>
                                        <td>{index + 1}</td>
                                        <td>{guest.name}</td>
                                        <td>{guest.email}</td>
...
export default AdminGetDailyActiveGuest;
