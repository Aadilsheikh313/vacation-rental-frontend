...
const AdminBanLogList = ({ userId, onUnbanNoteSubmit }) => {
    const dispatch = useDispatch();
    const [unbanNote, setUnbanNote] = useState("");
    const { banLogs, isLoading, isError, message } = useSelector(state => state.adminBannedUser);
    ...
    const filteredLogs = userId
        ? banLogs.filter(log => log.targetUser?._id === userId)
        : banLogs;

    if (isLoading) return <p>Loading logs...</p>;
    if (isError) return <p className="text-red-500">{message}</p>;
    if (!filteredLogs.length) return <p>No ban/unban logs available.</p>;
...
export default AdminBanLogList;
